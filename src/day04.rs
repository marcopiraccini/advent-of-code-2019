...
pub fn part1(input: String) {
    let numbers = input.trim().split('-').map(|x| x.parse::<usize>().unwrap()).collect::<Vec<usize>>(); 
    let res = (numbers[0]..numbers[1])
        .fold(0, | acc, n | if is_pwd(n) {acc + 1} else {acc}  );
...
pub fn part2(input: String) {
    let numbers = input.trim().split('-').map(|x| x.parse::<usize>().unwrap()).collect::<Vec<usize>>(); 
    let res = (numbers[0]..numbers[1])
        .fold(0, | acc, n | if is_pwd2(n) {acc + 1} else {acc}  );
    println!("Solution: {}", res);
}
